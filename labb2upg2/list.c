...
void print_list(const List list)
{
	Node* current = list;
	if(is_empty(list)) {
		char emptyMassage[] = "Noting in the list\r\n";
		uarte_write(emptyMassage, strlen(emptyMassage));
	}
	else{
		while (current != NULL) {
			char buffer[10];
...
//retura 1 om sorterad annas 0
int is_sorted_list(List *list) {
	if((*list)->data < (*list)->next && (*list)->next != NULL) {
...
static int* is_sorted_array(array[])
{
    ...
    int index = 0;
    sorted_to_array_help(&list, &array, &index);
    return array;
